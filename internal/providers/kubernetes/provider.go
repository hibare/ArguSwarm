...
// Provider implements the providers.Provider interface for Kubernetes.
...
func (p *Provider) GetType() providers.ProviderType {
	return providers.ProviderKubernetes
}
...
func (p *Provider) GetContainers(ctx context.Context) ([]providers.ContainerInfo, error) {
	...
	containers := make([]providers.ContainerInfo, 0)
	for _, pod := range pods.Items {
		for _, container := range pod.Spec.Containers {
			...
			containers = append(containers, providers.ContainerInfo{
...
func (p *Provider) GetImages(ctx context.Context) ([]providers.ImageInfo, error) {
	...
	imageMap := make(map[string]providers.ImageInfo)
	for _, pod := range pods.Items {
		for _, container := range pod.Spec.Containers {
			imageID := container.Image
			if _, exists := imageMap[imageID]; !exists {
				imageMap[imageID] = providers.ImageInfo{
					ID:       imageID,
	...
	images := make([]providers.ImageInfo, 0, len(imageMap))
...
func (p *Provider) GetNetworks(ctx context.Context) ([]providers.NetworkInfo, error) {
	...
	networks := make([]providers.NetworkInfo, 0, len(services.Items))
	for _, service := range services.Items {
		// Convert service ports to network info
		ports := make([]providers.IPAMConfig, 0, len(service.Spec.Ports))
		for _, port := range service.Spec.Ports {
			ports = append(ports, providers.IPAMConfig{
...
func (p *Provider) GetVolumes(ctx context.Context) ([]providers.VolumeInfo, error) {
	...
	volumes := make([]providers.VolumeInfo, 0, len(pvcs.Items))
